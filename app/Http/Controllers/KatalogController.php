...
class KatalogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories = KotaHasArtefakModel::select('kategori')->distinct()->get();
        $years = KotaHasArtefakModel::selectRaw('YEAR(waktu_pengumpulan) as year')
            ->distinct()
            ->orderBy('year', 'desc')
            ->get();
        $prodis = KotaHasArtefakModel::select('prodi')->distinct()->get();
        ...
        if ($request->filled('kategori')) {
            $query->where('tbl_kota_has_artefak.kategori', $request->kategori);
        }
    
        // Filter berdasarkan prodi
        if ($request->filled('prodi')) {
            $query->where('tbl_kota_has_artefak.prodi', $request->prodi);
        }
