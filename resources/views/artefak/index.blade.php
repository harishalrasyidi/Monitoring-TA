                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="kategori_artefak" id="kategori_artefak" class="form-control" required>
                                                        ...
                                                        {{-- Opsi Teks hanya akan ditampilkan via JS --}}
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                    ...
                                    <div class="col-md-4 mb-4">
                                        <div class="card h-100">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <form action="{{ isset($artefak->kumpul) ? route('submissions.updateTeks', $artefak->id_artefak) : route('submissions.store', $artefak->id_artefak) }}" method="POST">
                                                    @csrf

                                                    @if(isset($artefak->kumpul))
                                                        @method('PUT')
                                                    @endif

                                                    <div class="form-group">
                                                        <label for="teks_pengumpulan_{{ $artefak->id_artefak }}">
                                                            {{ isset($artefak->kumpul) ? 'Update Teks:' : 'Masukkan Teks:' }}
                                                        </label>
                                                        <textarea class="form-control" name="teks_pengumpulan" id="teks_pengumpulan_{{ $artefak->id_artefak }}" rows="4" placeholder="Masukkan teks abstrak TA di sini..." required>{{ isset($artefak->kumpul) ? $artefak->kumpul->teks_pengumpulan : '' }}</textarea>
                                                    </div>

                                                    <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-2">
                                                        <button class="btn btn-primary" type="submit">
                                                            {{ isset($artefak->kumpul) ? 'Update Teks' : 'Kumpulkan Teks' }}
                                                        </button>
                                                    </div>
                                                </form>
                                            @endif
...
@endsection
{{-- Script untuk menampilkan opsi "Teks" jika "Abstrak TA" dipilih --}}
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const namaArtefakSelect = document.getElementById('nama_artefak');
        const kategoriArtefakSelect = document.getElementById('kategori_artefak');

        function updateKategoriOptions() {
            const selectedNama = namaArtefakSelect.value;

            // Cek apakah opsi "Teks" sudah ada
            let teksOption = kategoriArtefakSelect.querySelector('option[value="Teks"]');

            if (selectedNama === 'Abstrak TA') {
                // Jika belum ada, tambahkan opsi Teks
                if (!teksOption) {
                    teksOption = document.createElement('option');
                    teksOption.value = 'Teks';
                    teksOption.textContent = 'Teks';
                    kategoriArtefakSelect.appendChild(teksOption);
                }
            } else {
                // Jika ada, hapus opsi Teks
                if (teksOption) {
                    teksOption.remove();
                }
            }
        ...
        namaArtefakSelect.addEventListener('change', updateKategoriOptions);
    });
</script>